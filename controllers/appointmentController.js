...
async function updateAppointmentStatus(req, res) {
  ...
  try {
    ...
    if (Status === "Confirmed") {
      ...
      const orderedAppointments = [...confirmedAppointments, appointment].sort(
        (a, b) => {
...
async function streamDoctorQueue(req, res) {
  try {
    ...
    console.error("Critical error inside SSE stream loop setup:", streamError);
    if (!res.headersSent) {
...
async function advanceToNextAppointment(req, res) {
  ...
  if (!DoctorID) {
    return res
      .status(400)
      .json({ message: "DoctorID is required to advance the correct queue." });
  }
  ...
  try {
    console.log("advanceToNextAppointment triggered for Doctor:", DoctorID);

    const staff = await Staff.findOne({ Email });
    if (!staff) {
      return res.status(404).json({ message: "Staff records not found" });
    }
    ...
    const completedAppointment = await Appointment.findOneAndUpdate(
      currentFilter,
      ...
      if (liveDoctorStreams[docKey]) {
        ...
        liveDoctorStreams[docKey].forEach((client) => {
          try {
            ...
            console.error("Bad client connection write dropped:", e);
          }
        });
      }
      return res
        .status(200)
        .json({ message: "No waiting confirmed appointments available" });
    }

    const exactDoctorObj = doctors.find(
    ...
    if (liveDoctorStreams[docKey]) {
      ...
      liveDoctorStreams[docKey].forEach((clientRes) => {
        try {
          clientRes.write(dataPayload);
        } catch (writeErr) {
          console.error(
            "Failed stream write to client window connection, dropping client...",
            writeErr,
          );
        }
      });
  ...
  try {
    ...
    activeConsultations.forEach((app) => {
      summary[app.DoctorID] = {
        ...
        CurrentLiveToken: app.TokenNumber,
